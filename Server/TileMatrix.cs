...
namespace Server
{
	...
	private Object SetStaticBlockLock = new Object();

	public void SetStaticBlock( int x, int y, StaticTile[][][] value )
	{
	    lock (SetStaticBlockLock){
	...
	private Object GetStaticBlockLock = new Object();
	public StaticTile[][][] GetStaticBlock( int x, int y )
	{
	    lock (GetStaticBlockLock){
	...
	private Object GetStaticTilesLock = new Object();
	public StaticTile[] GetStaticTiles( int x, int y, bool multis )
	{
	    lock (GetStaticTilesLock){
		StaticTile[][][] tiles = GetStaticBlock( x >> 3, y >> 3 );
	...
	private Object SetLandBlockLock = new Object();
	public void SetLandBlock( int x, int y, LandTile[] value )
	{
	    lock (SetLandBlockLock){
		if ( x < 0 || y < 0 || x >= m_BlockWidth || y >= m_BlockHeight )
		    return;
	...
	private Object GetLandBlockLock = new Object();
	public LandTile[] GetLandBlock( int x, int y )
	{
	    lock (GetLandBlockLock){
	...
	private Object ReadStaticBlockLock = new Object();
	private unsafe StaticTile[][][] ReadStaticBlock( int x, int y ) {
	    lock (ReadStaticBlockLock){
		try
		{
	...
	private Object ReadLandBlockLock = new Object();
	private unsafe LandTile[] ReadLandBlock( int x, int y ) {
	    lock (ReadLandBlockLock) {
		try
		{
